...
public class EnquiryController {
	...
	@GetMapping("/{id}")
	public String getEnqById(@PathVariable Long id) {
		String finalResp = null;
		EndResult result = null;
		try {
			EnquiryDetailsResponse updatedEnq = enquiryService.getEnquiryDetailsById(id);
			result= setEndResult(result,AppConst.SUCCESS, "Enquiry data Recieved",AppConst.ERRORCODE_SUCCESS);
			result.setData(updatedEnq);
			ObjectMapper mapper = new ObjectMapper();
			finalResp = mapper.writeValueAsString(result);
			log.info("In RecieveEnquiryById(), finalResp:"+finalResp);
		} catch (Exception e) {
			result= setEndResult(result,AppConst.FAILURE, e.getMessage(),AppConst.ERRORCODE_EXCEPTION);
			finalResp = new Gson().toJson(result);
			log.error("In RecieveEnquiryById(), finalResp:"+finalResp);
			return finalResp;
		}
		return finalResp;
	}
    ...
    public String uploadEnquiries(@RequestBody List<EnquiryReq> enquirires) {
        String finalResp;
        EndResult result;
        try {
        	List<Enquiry> importedEnquiries =enquiryService.saveAll(enquirires);
            result = setEndResult(null, AppConst.SUCCESS, "Enquiries imported successfully", AppConst.ERRORCODE_SUCCESS);
            result.setData(importedEnquiries);
            finalResp = new ObjectMapper().writeValueAsString(result);
            log.info("In bulk-uploadenquiry(), finalResp:"+finalResp);
        } catch (Exception e) {
